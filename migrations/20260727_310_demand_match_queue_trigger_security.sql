-- Property writes run with the authenticated user's RLS context. The matching
-- queue is deliberately server-only, so its trigger must perform only this
-- narrow enqueue operation with the migration owner's privileges.
