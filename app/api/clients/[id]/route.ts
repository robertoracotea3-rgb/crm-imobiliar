...
export async function GET(
  ...
  const { data: contact, error: contactError } = await admin
    .from('contacts')
    .select('id, full_name, phone, phone_secondary, email, cnp, address, type, notes, source, agent_id, gdpr_consent, gdpr_consent_at, merge_status, merged_into_id, created_at, updated_at')
    .eq('id', id)
  ...
  const [leadResult, demandResult, activityResult, viewingResult, transactionResult, ownedPropertyResult, identifierResult, sourceResult, agentResult] = await Promise.all([
