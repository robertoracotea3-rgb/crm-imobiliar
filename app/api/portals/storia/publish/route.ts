    ...
    await supabase.from('portal_listings').upsert(
      {
        ...
        last_error_code: `publish_http_${storiaResponse.status}`,
        last_check_payload: { status: 'error', operation: method },
  ...
  await supabase.from('portal_listings').upsert(
    {
      ...
      last_error_code: errorMessage ? 'publish_validation_error' : null,
      last_check_payload: {
        operation: method,
        external_id: externalId,
        portal_ad_id: portalAdId,
