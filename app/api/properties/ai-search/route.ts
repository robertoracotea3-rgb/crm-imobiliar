...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!query?.trim()) return Response.json({ error: 'Interogare goala' }, { status: 400 });

    if (!process.env.ANTHROPIC_API_KEY) {
      return Response.json({ error: 'Cautarea AI nu este configurata (lipseste ANTHROPIC_API_KEY).' }, { status: 503 });
    }

    const client = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });
    const sys = `Esti un asistent care transforma cautari imobiliare in limba romana in filtre JSON.
Raspunde doar cu un obiect JSON valid, fara text suplimentar, cu cheile:
- category: una din [${VALID_CATEGORY.join(', ')}] sau null
- transaction: "vanzare" | "inchiriere" | null
- county: numele judetului sau null
- city: numele localitatii sau null
- price_min: numar sau null
- price_max: numar sau null
- rooms_min: numar sau null
- rooms_max: numar sau null
- surface_min: numar (mp) sau null`;
    ...
    try {
      ...
      return Response.json({ error: `Eroare AI: ${e instanceof Error ? e.message : 'necunoscuta'}` }, { status: 500 });
    }
    ...
    let results = data || [];
    const roomsOf = (p: { attributes?: Record<string, unknown> }) => parseFloat(String(p.attributes?.nr_camere ?? ''));
    const surfOf = (p: { attributes?: Record<string, unknown> }) => parseFloat(String(p.attributes?.sup_utila ?? p.attributes?.sup_construita ?? ''));
