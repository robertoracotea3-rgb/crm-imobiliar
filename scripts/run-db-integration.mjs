...
const database = 'crmtest_phase22_automated';
...
try {
  ...
  const output = psql(database, readFileSync('tests/integration/phase22-workflow.sql', 'utf8'));
  if (!output.includes('phase22_db_integration_ok')) {
    throw new Error(`Database integration marker is missing.\n${output}`);
  }
  console.log('Database integration passed: viewing, transaction finalization and portal removal.');
} finally {
  try {
